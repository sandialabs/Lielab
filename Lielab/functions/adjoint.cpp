...
Lielab::domain::glr ad(const Lielab::domain::se& a, const int p)
{
    ...
    */
    ...
    if (shape == 3)
    {
        ...
        {
            out.point(0, 0) = -theta2;
            out.point(1, 1) = -theta2;
            out.point(0, 2) = theta*x;
            out.point(1, 2) = theta*y;
        }
        
        out.point = coeff*out.point;
        return out;
    }
...
Lielab::domain::se ad(const Lielab::domain::se& a, const Lielab::domain::se& b, const int p)
{
    ...
    */
    ...
    if (shape == 3 || shape == 4)
    {
        ...
        const Eigen::VectorXd bbar = b.get_vector();
        Lielab::domain::se out = Lielab::domain::se::zero(shape);
        out.set_vector(adjaphat*bbar);
        return out;
    }
...
Lielab::domain::glr ad(const Lielab::domain::so& a, const int p)
{
    ...
    */
    ...
    if (shape == 2)
    {
        return Lielab::domain::glr::zero(dim);
    }
...
template <>
Lielab::domain::CompositeAlgebra commutator(const Lielab::domain::CompositeAlgebra& x, const Lielab::domain::CompositeAlgebra& y)
{
    /*!
    ...
    */
    ...
    lielab_assert(check_topology(x, y), "Unable to take commutator of topologically inconsistent algebras: (" + x.to_string() + ") !≅ (" + y.to_string() + ").");
    ...
    for (const auto& element : x.point)
    {
        std::visit([&](const auto& _element)
        {
            ...
            out.point.push_back(commutator(_element, std::get<other_t>(y.point[index])));
        }, element);
        index++;
    }
...
template <>
Lielab::domain::CompositeAlgebra ad_numerical(const Lielab::domain::CompositeAlgebra& x, const int power)
{
    /*!
    * CompositeAlgebra ad_numerical overload.
    */
    ...
    for (const auto& element : x.point)
    {
        std::visit([&](const auto& _element)
        {
            out.point.push_back(ad_numerical(_element, power));
        }, element);
    }
...
template <>
Lielab::domain::CompositeAlgebra ad(const Lielab::domain::CompositeAlgebra& x, const int power)
{
    /*!
    ...
    */
    ...
    for (const auto& element : x.point)
    {
        std::visit([&](const auto& _element)
        {
            out.point.push_back(ad(_element, power));
        }, element);
    }
...
template <>
Lielab::domain::CompositeAlgebra ad_numerical(const Lielab::domain::CompositeAlgebra& x, const Lielab::domain::CompositeAlgebra& y, const int power)
{
    /*!
    * CompositeAlgebra ad overload.
    */
    ...
    lielab_assert(check_topology(x, y), "Unable to take numerical adjoint of topologically inconsistent algebras: (" + x.to_string() + ") !≅ (" + y.to_string() + ").");
    ...
    for (const auto& element : x.point)
    {
        ...
        {
            using other_t = std::decay_t<decltype(_element)>;
            out.point.push_back(ad_numerical(_element, std::get<other_t>(y.point[index]), power));
        }, element);
        index++;
    }
...
template <>
Lielab::domain::CompositeAlgebra ad(const Lielab::domain::CompositeAlgebra& x, const Lielab::domain::CompositeAlgebra& y, const int power)
{
    /*!
    * CompositeAlgebra ad overload.
    */
    ...
    lielab_assert(check_topology(x, y), "Unable to take adjoint of topologically inconsistent algebras: (" + x.to_string() + ") !≅ (" + y.to_string() + ").");
    ...
    for (const auto& element : x.point)
    {
        ...
        {
            using other_t = std::decay_t<decltype(_element)>;
            out.point.push_back(ad(_element, std::get<other_t>(y.point[index]), power));
        }, element);
        index++;
    }
...
template <>
Lielab::domain::CompositeAlgebra coad_numerical(const Lielab::domain::CompositeAlgebra& x, const int power)
{
    /*!
    * CompositeAlgebra coad_numerical overload.
    */
    ...
    for (const auto& element : x.point)
    {
        std::visit([&](const auto& _element)
        {
            out.point.push_back(coad_numerical(_element, power));
        }, element);
    }
...
template <>
Lielab::domain::CompositeAlgebra coad(const Lielab::domain::CompositeAlgebra& x, const int power)
{
    /*!
    * CompositeAlgebra coad overload.
    */
    ...
    for (const auto& element : x.point)
    {
        std::visit([&](const auto& _element)
        {
            out.point.push_back(coad(_element, power));
        }, element);
    }
...
template <>
Lielab::domain::CompositeAlgebra coad_numerical(const Lielab::domain::CompositeAlgebra& x, const Lielab::domain::CompositeAlgebra& y, const int power)
{
    /*!
    * CompositeAlgebra coad overload.
    */
    ...
    lielab_assert(check_topology(x, y), "Unable to take numerical coadjoint of topologically inconsistent algebras: (" + x.to_string() + ") !≅ (" + y.to_string() + ").");
    ...
    for (const auto& element : x.point)
    {
        ...
        {
            using other_t = std::decay_t<decltype(_element)>;
            out.point.push_back(coad_numerical(_element, std::get<other_t>(y.point[index]), power));
        }, element);
        index++;
    }
...
template <>
Lielab::domain::CompositeAlgebra coad(const Lielab::domain::CompositeAlgebra& x, const Lielab::domain::CompositeAlgebra& y, const int power)
{
    /*!
    * CompositeAlgebra coad overload.
    */
    ...
    lielab_assert(check_topology(x, y), "Unable to take coadjoint of topologically inconsistent algebras: (" + x.to_string() + ") !≅ (" + y.to_string() + ").");
    ...
    for (const auto& element : x.point)
    {
        ...
        {
            using other_t = std::decay_t<decltype(_element)>;
            out.point.push_back(coad(_element, std::get<other_t>(y.point[index]), power));
        }, element);
        index++;
    }
...
template <>
Lielab::domain::CompositeAlgebra Ad(const Lielab::domain::CompositeGroup& g, const Lielab::domain::CompositeAlgebra& x)
{
    /*!
    * CompositeAlgebra Ad overload.
    */
    ...
    // TODO: How do we do real time checks for this?
    // lielab_assert(check_topology(g, x), "Unable to take numerical coadjoint of topologically inconsistent ???: (" + g.to_string() + ") ?? (" + x.to_string() + ").");
    ...
    for (const auto& element : g.point)
    {
        std::visit([&](const auto& _element)
        {
            using other_t = LieIII<std::decay_t<decltype(_element)>>;
            out.point.push_back(Ad(_element, std::get<other_t>(x.point[index])));
        }, element);
        index++;
    }
