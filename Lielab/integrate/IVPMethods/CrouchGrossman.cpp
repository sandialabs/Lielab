...
CrouchGrossman::CrouchGrossman(const CrouchGrossmanCoefficients tableau)
{
    /*!
    * Instantiates a new CrouchGrossman object with a specific CG method.
    */
    
    const auto [_A, _b, _bhat, _c, _e, _order, _stages, _variable, _implicit] = get_crouch_grossman_coefficients(tableau);
...
void CrouchGrossman::estimate_error(const Eigen::VectorXd& yl, const Eigen::VectorXd& yh, const double dt)
{
    const Eigen::VectorXd scale = this->abstol + this->reltol*yl.array().max(yh.array()).cwiseAbs();
    const size_t sz = yl.size();

    const Eigen::VectorXd err0 = (yh.array() - yl.array()).cwiseAbs();

    const Eigen::VectorXd err = err0.array()/scale.array();
    this->error_estimate = err.norm()/std::sqrt(static_cast<double>(sz));
}

Lielab::domain::CompositeManifold CrouchGrossman::operator()(const HomogeneousIVPSystem& dynamics, const Lielab::domain::CompositeManifold& y0, const double t0, const double dt)
{
    /*!
     * Primary usage of CrouchGrossman.
     */

    const Lielab::domain::CompositeAlgebra dy0 = dynamics.generator(t0, y0);
    Lielab::domain::CompositeAlgebra dytemp = dy0*0.0;
    ...
    if (dy0bar.array().isNaN().any())
    {
        ...
        this->message = "CrouchGrossman: NaNs in generator.";
        this->success = false;
        return y0;
    }
    ...
    if (dy0bar.array().isInf().any())
    {
        ...
        this->message = "CrouchGrossman: Infs in generator.";
        this->success = false;
        return y0;
    }

    const size_t n_eoms = dy0bar.size();
    ...
    double next_t = t0;
    Lielab::domain::CompositeManifold next_y = y0;
    ...
    if (!this->implicit)
    {
        for (int stage = 0; stage < this->n; stage++)
        {
            ...
            else
            {
                ...
                {
                    dytemp.set_vector(dt*this->A(stage, jj)*this->K.row(jj));
                    next_y = dynamics.action(dynamics.coordinates(dytemp), next_y);
                }
            }

            this->K.row(stage) = dynamics.generator(next_t, next_y).get_vector();
        }
    ...
    {
        this->status = IVPStatus::ERROR; // placeholder error
        this->message = "CrouchGrossman: Implicit methods not implemented.";
        this->success = false;
        return y0;
        ...
        //         Knew.row(stage) = dynamics.connection(next_theta, dynamics.vectorfield(next_t, dynamics.action(dynamics.coordinates(next_theta), y0))).get_vector();
        //     }
        ...
        //     {
        //         this->status = IVPStatus::ERROR_NANS_IN_VF;
        //         this->message = "CrouchGrossman: NaNs in vectorfield.";
        //         this->success = false;
        //         return y0;
        //     }
        ...
        //     {
        //         this->status = IVPStatus::ERROR_INFS_IN_VF;
        //         this->message = "CrouchGrossman: Infs in vectorfield.";
        //         this->success = false;
        //         return y0;
        //     }
    ...
    for (int ii = 0; ii < this->n; ii++)
    {
        dytemp.set_vector(dt*this->B(ii)*this->K.row(ii));
        next_y = dynamics.action(dynamics.coordinates(dytemp), next_y);
    }
    ...
    {
        Lielab::domain::CompositeManifold next_y2 = y0;
        for (int ii = 0; ii < this->n; ii++)
        {
            dytemp.set_vector(dt*this->Bhat(ii)*this->K.row(ii));
            next_y2 = dynamics.action(dynamics.coordinates(dytemp), next_y2);
        }
        this->estimate_error(next_y.serialize(), next_y2.serialize(), dt);
    }

    this->status = IVPStatus::SUCCESS;
    this->success = true;
    this->message = "CrouchGrossman: Terminated successfully.";
...
IVPSolution CrouchGrossmanFlow::operator()(const HomogeneousIVPSystem& dynamics, const Eigen::VectorXd& tspan, const Lielab::domain::CompositeManifold& y0, const IVPOptions& options)
{
    /*!
    ...
    */
    ...
    CrouchGrossman solver = CrouchGrossman(options.crouch_grossman_coefficients);
    ...
    while (computestatus == 1 || computestatus == 2)
    {
            ...
            if (has_event)
            {
                ...
                if ((event_current >= 0) && (event_next <= 0))
                {
                    ...
                    if (!this->tolerance_not_met)
                    {
                        ...
                        this->message = "CrouchGrossmanFlow: Event triggered.";
                        this->success = true;
                        out.trim_chunk();
                        return out;
                    }
                    else
                    {
                        ...
                        this->message = "CrouchGrossmanFlow: Event triggered, but integration tolerance was not met.";
                        this->success = true;
                        out.trim_chunk();
                        return out;
                    }
        ...
        else if (computestatus == 2)
        {
            ...
            if (t - tf > -options.abstol || tind >= tspan.size())
            {
                ...
                {
                    this->status = IVPStatus::SUCCESS;
                    this->message = "CrouchGrossmanFlow: Final time reached.";
                    this->success = true;
                    out.trim_chunk();
                    return out;
                }
                else
                {
                    ...
                    this->message = "CrouchGrossmanFlow: Final time reached, but integration tolerance was not met.";
                    this->success = true;
                    out.trim_chunk();
                    return out;
                }
            ...
            else if (options.max_iterations > 0 && this->iterations >= options.max_iterations)
            {
                ...
                this->message = "CrouchGrossmanFlow: Max iterations reached.";
                this->success = false;
                out.trim_chunk();
                return out;
            }
    ...
    this->message = "CrouchGrossmanFlow: Unknown error.";
    this->success = false;
    out.trim_chunk();
    return out;
}
