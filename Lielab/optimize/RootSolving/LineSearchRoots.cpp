...
LineSearchRoots::LineSearchRoots()
{

}

Eigen::VectorXd LineSearchRoots::operator()(const EuclideanRootSystem system,
                                       const Eigen::VectorXd& guess,
                                       const Eigen::VectorXd& direction,
                                       const double magnitude,
                                       const RootSolvingOptions options)
{
    this->iteration = 0;
    // this->status = 
    this->success = false;
    ...
    // Pass 1
    this->iteration++; // TODO: Remove?

    // Pass 2
    this->iteration++;
    x1 = guess;
    y1 = system.objective(x1);
    ...
    //     // Account for bounds
    //     for (int ii = 0; ii < static_cast<int>(options.lower_bound.size()); ii++)
    //     {
    //         if (!std::isnan(options.lower_bound(ii)))
    //         {
    //             xnew(ii) = std::max(xnew(ii), options.lower_bound(ii));
    //         }
    //     }

    //     for (int ii = 0; ii < static_cast<int>(options.upper_bound.size()); ii++)
    //     {
    //         if (!std::isnan(options.upper_bound(ii)))
    //         {
    //             xnew(ii) = std::min(xnew(ii), options.upper_bound(ii));
    //         }
    //     }
