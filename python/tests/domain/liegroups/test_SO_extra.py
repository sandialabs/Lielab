...
def test_SO():
    ...
    # Group product
    x*y
    check_almost_equal_nulp(CompositeGroup([x*y]), CompositeGroup([(y.inverse()*x.inverse()).inverse()]), 1, True)
...
def test_from_SU2():
    ...
    from lielab.domain import CompositeGroup, SO, SU, su
    from lielab.functions import exp
    from lielab.testing import check_almost_equal_nulp

    # Build 90 degree rotations in x, y, and z
    u = su.from_vector([0.0, 0.0, 0.5])
    v = su.from_vector([0.0, 0.5, 0.0])
    w = su.from_vector([0.5, 0.0, 0.0])
    qx = exp(np.pi/2.0*u)
    qy = exp(np.pi/2.0*v)
    qz = exp(np.pi/2.0*w)
    ...
    assert check_almost_equal_nulp(CompositeGroup([rx]), CompositeGroup([DCMrotx]), 4, True)

    # Test 90 degree y rotation
    ry = SO.from_SU2(qy)

    assert check_almost_equal_nulp(CompositeGroup([ry]), CompositeGroup([DCMroty]), 4, True)

    # Test 90 degree z rotation
    rz = SO.from_SU2(qz)

    assert check_almost_equal_nulp(CompositeGroup([rz]), CompositeGroup([DCMrotz]), 4, True)
...
def test_from_eulerangles_body123():
    ...
    assert check_almost_equal_nulp(CompositeGroup([DCMId]), CompositeGroup([dcm]), 0, True)

    # Rotate 90 degrees by x-axis
    dcm = SO.from_eulerangles_body123(np.pi/2.0, 0.0, 0.0)
    assert check_almost_equal_nulp(CompositeGroup([DCMrotx]), CompositeGroup([dcm]), 4, True)

    # Rotate 90 degrees by y-axis
    dcm = SO.from_eulerangles_body123(0.0, np.pi/2.0, 0.0)
    assert check_almost_equal_nulp(CompositeGroup([DCMroty]), CompositeGroup([dcm]), 4, True)

    # Rotate 90 degrees by z-axis
    dcm = SO.from_eulerangles_body123(0.0, 0.0, np.pi/2.0)
    assert check_almost_equal_nulp(CompositeGroup([DCMrotz]), CompositeGroup([dcm]), 4, True)

    # Rotate by a random angle
    dcm = SO.from_eulerangles_body123(some_angle, some_angle, some_angle)
    assert_array_max_ulp(np.linalg.det(dcm.get_matrix()), 1.0, maxulp=4)
    assert_array_max_ulp((dcm.get_matrix() @ dcm.get_matrix().transpose()).trace(), 3.0, maxulp=4)
...
def test_to_eulerangles_body123():
    ...
    assert check_almost_equal_nulp(theta1, 0.0, 1, True)
    assert check_almost_equal_nulp(theta2, 0.0, 1, True)
    assert check_almost_equal_nulp(theta3, 0.0, 1, True)

    # Rotate 90 degrees by x-axis
    theta1, theta2, theta3 = DCMrotx.to_eulerangles_body123()
    assert check_almost_equal_nulp(theta1, np.pi/2.0, 1, True)
    assert check_almost_equal_nulp(theta2, 0.0, 1, True)
    assert check_almost_equal_nulp(theta3, 0.0, 1, True)
    ...
    assert check_almost_equal_nulp(theta1, 0.0, 1, True)
    assert check_almost_equal_nulp(theta2, np.pi/2.0, 1, True)
    assert check_almost_equal_nulp(theta3, 0.0, 1, True)
    ...
    assert check_almost_equal_nulp(theta1, some_angle, 1, True)
    assert check_almost_equal_nulp(theta2, some_angle, 1, True)
    assert check_almost_equal_nulp(theta3, some_angle, 1, True)
